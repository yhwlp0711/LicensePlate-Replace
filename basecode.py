...
def replaceImgBySpecImgRefPoint(largeImg, largeImgRefPoint, smallImg, smallImgRefPoint, overlyBlackMaxValue=None):
    ...
    if lx < 0 or ly < 0 or lx2 > lw or ly2 > lh:
        raise ValueError(
            f'largeImg图像对应参考点{largeImgRefPoint}和smallImg图像对应参考点{smallImgRefPoint}重叠时smallImg图像超出了largeImg图像范围')
...
def constructAffineMatrix(rotationAngle=0, xShearAngle=0, yShearAngle=0, translationX=0, translationY=0, scaleX=1,
                          scaleY=1):
    """
    ...
    if rotationAngle:
        ...
        return np.float32(
            [[cos(rotationAngle), -sin(rotationAngle), 0], [sin(rotationAngle), cos(rotationAngle), 0], [0, 0, 1]])
    elif xShearAngle:
        ...
        return np.float32([[scaleX, 0, 0], [0, scaleY, 0], [0, 0, 1]])
